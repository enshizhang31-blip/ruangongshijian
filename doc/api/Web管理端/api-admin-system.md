...
┌─────────────────────────────────────────────────────────────────────┐
│  员工                                                               │
│  - permissions: 权限数组（从角色复制，之后独立）                        │
│  - routes: 路由数组（从角色复制，之后独立）                           │
│                                                                  │
│  有效权限 = 员工的 permissions                                       │
│  有效路由 = 员工的 routes                                           │
│                                                                  │
│  ⚠️ 员工表中不存储 role_id，创建时Java从角色复制                      │
└─────────────────────────────────────────────────────────────────────┘
```
...
| phone | string | 否 | 手机号 |
| email | string | 否 | 邮箱 |
| roleId | long | 是 | 角色ID（Java代码获取权限并复制到员工） |
...
1. 校验用户名唯一性
2. 密码使用BCrypt加密存储
3. Java从角色模板复制permissions和routes到员工记录
4. 记录操作日志
...
| phone | string | 否 | 手机号 |
| email | string | 否 | 邮箱 |
| roleId | long | 否 | 角色ID（仅创建时使用，之后不存储） |
| departmentId | long | 否 | 部门ID |
...
- 创建时：Java从角色模板复制permissions和routes到员工
- 员工表中不存储role_id
...
1. **角色只是模板**：预设角色不可修改，创建员工时Java从角色复制权限
2. **权限在员工上**：创建后权限独立存储在员工表中，不与角色关联
3. **计算公式**：
   - 有效权限 = 员工的 permissions
   - 有效路由 = 员工的 routes
4. 操作日志记录保留180天