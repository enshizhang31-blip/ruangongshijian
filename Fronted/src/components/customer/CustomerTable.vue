...
const typeMap: Record<number, { label: string; class: string }> = {
    1: { label: '个人', class: 'bg-blue-100 text-blue-700' },
    2: { label: '企业', class: 'bg-purple-100 text-purple-700' },
}
</script>
        ...
        <table class="w-full bg-white rounded-xl text-sm min-w-max">
            <thead class="bg-gray-50">
                <tr class="text-left text-sm text-gray-600">
                    <th class="px-6 py-3 font-medium">客户名称</th>
                    <th class="px-6 py-3 font-medium">联系方式</th>
                    <th class="px-6 py-3 font-medium">客户类型</th>
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr v-for="item in list" :key="item.id"
                    ...
                    <td class="px-6 py-4">
                        <span class="px-2 py-1 rounded-full text-xs" :class="typeMap[item.customerType]?.class">
                            {{ typeMap[item.customerType]?.label || '未知' }}
                        </span>
                    </td>
                    <td class="px-6 py-4 text-gray-500">{{ item.level || '-' }}</td>
                    <td class="px-6 py-4 text-blue-600 font-medium">¥{{ item.balance || 0 }}</td>
                    <td class="px-6 py-4">
