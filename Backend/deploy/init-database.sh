...
docker exec -i "$MYSQL_CONTAINER" mysql -u"$MYSQL_USER" -p"$MYSQL_PASSWORD" "$MYSQL_DATABASE" < "$SCRIPT_DIR/sql/schema.sql"
echo "OK"

echo ""
echo "===== 2. MySQL 种子数据 (DML) ====="
docker exec -i "$MYSQL_CONTAINER" mysql -u"$MYSQL_USER" -p"$MYSQL_PASSWORD" "$MYSQL_DATABASE" < "$SCRIPT_DIR/sql/seed-data.sql"
echo "OK"

echo ""
echo "===== 3. MongoDB 初始化 ====="
docker exec -i "$MONGO_CONTAINER" mongosh -u "$MONGO_USER" -p "$MONGO_PASSWORD" --authenticationDatabase admin --quiet < "$SCRIPT_DIR/mongo-init.js"
echo "OK"
