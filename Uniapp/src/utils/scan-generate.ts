/**
 * 扫码生成工具 - 根据字符串生成二维码或条形码（Canvas 渲染，返回 dataURL）
 */
...
function encodeData(text: string, version: number): number[] {
  const result: number[] = []
  // 模式指示符: 0100 (字节模式)
  // 字符计数指示符长度 (版本 1-9: 8 bits, 10-26: 16 bits)
...
function buildMatrix(version: number, dataWithEC: number[]): number[][] {
  ...
  while (col > 0) {
    if (col === 6) col-- // 跳过垂直 timing pattern
